...
let currentRepeatItem;let data;
...
function showQuestion() {
  ...
  updateRepeatButton("mark-repeat", {
    type: "quiz",
...
function showYear() {
  ...
  updateRepeatButton("mark-repeat-year", {
    type: "year",
...
function updateRepeatButton(id, item) {
  const btn = document.getElementById(id);
  const exists = repeatItems.some(
    r => r.type === item.type && r.year === item.year
  );

  btn.textContent = exists ? "Repetera ✔️" : "Repetera?";
  btn.classList.toggle("active-repeat", exists);

  btn.onclick = () => {
    toggleRepeat(item);
    updateRepeatButton(id, item);
  };
}
...
function showRepeat() {
  ...
  repeatQuestionEl.textContent =
    currentRepeatItem.type === "quiz"
      ? currentRegent.questions.find(q => q.year === currentRepeatItem.year)?.q
